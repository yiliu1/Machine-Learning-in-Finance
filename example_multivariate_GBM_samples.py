...
def main():
    ...
    N=np.shape(prices)[1]
    logReturns = np.log(prices[:,1:N]) - np.log(prices[:,0:N-1])
    
    print("Estimated annualized covariance: \n" + np.array2string(np.cov(logReturns)*nBusDays))
    print("Estimated annualized instantaneous drift: \n" + np.array2string(np.mean(logReturns, axis=1, keepdims=True)*nBusDays))
